...
struct SettingsView: View {
    ...
    private var meshBlurb: String {
        "Runs a small peer-to-peer node that discovers the other Co-Maintainer machines on "
    ...
    private var updateBlurb: String {
        "Pulls the latest applet from GitHub, rebuilds the co-maintainer-core prompt engine and the "
    ...
    private var allocatorHint: String {
        if !DeviceAllocator.packageAvailable {
            return "Package not found at \(DeviceAllocator.packageDir). Set CO_MAINTAINER_DEVICE_ALLOCATOR_DIR to point at it."
        }
        if !DeviceAllocator.nodeAvailable {
            return "Node.js not found. Install Node (or set CO_MAINTAINER_NODE) — the allocator's MCP server and daemon need it to run."
        }
