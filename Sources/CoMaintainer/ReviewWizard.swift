...
// CoMaintainerCore (driven by core/review.json) and are shared verbatim with the
...
enum AgentSpawner {
    ...
    /// the tracking self-test (`CO_MAINTAINER_TRACK_TEST`). `restoreFocusTo` bounces
    ...
    static func writePrompt(_ prompt: String) throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("co-maintainer-review-\(UUID().uuidString).txt")
    ...
    static func doneFilePath() -> String {
        ...
        return dir.appendingPathComponent("co-maintainer-done-\(UUID().uuidString).txt").path
    }
