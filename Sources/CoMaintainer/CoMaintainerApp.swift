...
@main
struct CoMaintainerApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var delegate
    @StateObject private var store = Store()

    var body: some Scene {
        MenuBarExtra("Co-Maintainer", systemImage: "wrench.and.screwdriver") {
            PopoverRoot()
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // CO_MAINTAINER_PRINT_PROMPT=mine|user (default mine).
        if let mode = env["CO_MAINTAINER_PRINT_PROMPT"] {
        ...
        // uses the same `com.ignacy.co-maintainer` defaults domain as the GUI.
        if env["CO_MAINTAINER_SETTINGS_DUMP"] == "1" {
        ...
        if let what = env["CO_MAINTAINER_RENDER"] {
        ...
        if env["CO_MAINTAINER_TRACK_TEST"] == "1" {
        ...
        if env["CO_MAINTAINER_DEVICE_DUMP"] == "1" {
        ...
        if env["CO_MAINTAINER_AUTOFIX_POLL"] == "1" {
        ...
        if env["CO_MAINTAINER_APIWATCH_SCAN"] == "1" {
        ...
        if env["CO_MAINTAINER_SPAWN_FOCUS_TEST"] == "1" {
...
enum QuitFlow {
    @MainActor static func confirm() {
        ...
        let alert = NSAlert()
        alert.messageText = "Quit Co-Maintainer?"
...
enum SingleInstance {
    static let bundleID = "com.ignacy.co-maintainer"
    static let execName = "CoMaintainer"
    ...
    private static func log(_ msg: String) {
        FileHandle.standardError.write(Data("CoMaintainer singleton: \(msg)\n".utf8))
    }
}
...
enum Dump {
    ...
    /// unchanged from before this fix (see CO_MAINTAINER_PRINT_PROMPT) — that identity is
    ...
    @MainActor static func spawnFocusTest() -> Bool {
        ...
        let cmd1 = "echo 'co-maintainer bg focus self-test — closes itself'; sleep 2; printf %s $? > '\(done1)'"
        ...
        let cmd2 = "echo 'co-maintainer fg focus self-test — closes itself'; sleep 2; printf %s $? > '\(done2)'"
    ...
    /// `com.ignacy.co-maintainer` defaults domain.
