...
final class Store: ObservableObject {
    ...
    /// Live Co-Maintainer Mesh topology (the local node's `state.json` snapshot; nil until a node
    ...
    /// `CO_MAINTAINER_REFRESH_SECS` (clamped to ≥5s) for tuning/testing.
    static var autoRefreshInterval: TimeInterval {
        let secs = ProcessInfo.processInfo.environment["CO_MAINTAINER_REFRESH_SECS"].flatMap(Double.init)
    ...
    /// with `CO_MAINTAINER_PROC_POLL_SECS` (clamped ≥2s) for tuning/testing.
    static var processPollInterval: TimeInterval {
        let secs = ProcessInfo.processInfo.environment["CO_MAINTAINER_PROC_POLL_SECS"].flatMap(Double.init)
        return max(2, secs ?? 8)
    }
    ...
    static var autofixPollInterval: TimeInterval {
        let secs = ProcessInfo.processInfo.environment["CO_MAINTAINER_AUTOFIX_SECS"].flatMap(Double.init)
    ...
    static var apiWatchInterval: TimeInterval {
        let secs = ProcessInfo.processInfo.environment["CO_MAINTAINER_APIWATCH_SECS"].flatMap(Double.init)
    ...
    // MARK: - Co-Maintainer Mesh (LAN P2P topology)
    ...
    static var meshPollInterval: TimeInterval {
        let secs = ProcessInfo.processInfo.environment["CO_MAINTAINER_MESH_POLL_SECS"].flatMap(Double.init)
        return max(1, secs ?? 2)
    }
    ...
    /// Pull the checkout, rebuild `CoMaintainer.app`, relaunch it. The relaunched instance
