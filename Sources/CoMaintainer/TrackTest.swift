...
/// `CO_MAINTAINER_TRACK_TEST=1`. It exercises the *real* code the applet uses —
...
///   CO_MAINTAINER_TRACK_TEST=1 /Applications/CoMaintainer.app/Contents/MacOS/CoMaintainer
enum TrackTest {
    ...
    static func run() async -> Bool {
        ...
        let sentinel = NSTemporaryDirectory() + "co-maintainer-tracktest-\(UUID().uuidString)"
    ...
    private static func liveCycle(check: (String, Bool) -> Void) async {
        ...
        let cmd = "echo 'co-maintainer tracking self-test — this window closes itself'; "
