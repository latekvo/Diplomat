...
struct PopoverRoot: View {
    ...
    /// `CO_MAINTAINER_POPOVER_CAP` forces a small cap so the scrolling state is
    /// reproducible in the `popover` render self-test.
    private var cap: CGFloat {
        if let s = ProcessInfo.processInfo.environment["CO_MAINTAINER_POPOVER_CAP"],
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            if query.isEmpty, let pre = ProcessInfo.processInfo.environment["CO_MAINTAINER_PREFILL"], !pre.isEmpty {
                query = pre
                searchFocused = true
            }
    ...
    private var header: some View {
        ...
        return HStack(spacing: 6) {
            ...
            Text("Co-Maintainer").font(.headline)
