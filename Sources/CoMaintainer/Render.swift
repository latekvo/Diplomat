...
/// existing dump/lookup self-tests. Driven by `CO_MAINTAINER_RENDER=<what>` and
/// `CO_MAINTAINER_RENDER_OUT=<path>` (defaults under the temp dir).
@MainActor
enum Render {
    ...
    static func run(_ what: String, store: Store) -> Bool {
        let out = ProcessInfo.processInfo.environment["CO_MAINTAINER_RENDER_OUT"]
            ?? FileManager.default.temporaryDirectory.appendingPathComponent("co-maintainer-\(what).png").path
    ...
    /// `CO_MAINTAINER_RENDER=popover` — snapshot the REAL popover root in a live NSWindow
    ...
    /// Pair with `CO_MAINTAINER_POPOVER_CAP` (e.g. 400) to force the scrolling state.
    ...
    /// For `CO_MAINTAINER_RENDER=panel-procs`, inject a couple of fake tracked
    ...
    /// renders (CO_MAINTAINER_RENDER=settings).
    @MainActor
    ...
    /// eyeballed (CO_MAINTAINER_RENDER=wizard-banned).
    @MainActor
    ...
    /// Synthetic device-allocator state for `CO_MAINTAINER_RENDER=devices`.
