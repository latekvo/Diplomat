...
// and (with CO_MAINTAINER_DUMP=1) runs the real gh pipeline so the Swift core can
...
let cleanFiles = ["packages/co-maintainer-core/src/foo.ts", "README.md"]
...
// Regenerate after an intentional core/*.json change: CO_MAINTAINER_GOLDEN_WRITE=1 swift run
...
if ProcessInfo.processInfo.environment["CO_MAINTAINER_GOLDEN_WRITE"] == "1" {
    ...
    for (name, prompt) in goldenModes {
        ...
        check(!golden.isEmpty, "missing golden \(name).txt — run CO_MAINTAINER_GOLDEN_WRITE=1")
...
if ProcessInfo.processInfo.environment["CO_MAINTAINER_DUMP"] == "1" {
