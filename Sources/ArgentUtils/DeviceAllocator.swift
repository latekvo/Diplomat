...
enum DeviceAllocator {
    ...
    static var packageDir: String {
        if let env = ProcessInfo.processInfo.environment["ARGENT_DEVICE_ALLOCATOR_DIR"], !env.isEmpty {
            return env
        }
        return home.appendingPathComponent("dev/argent-utils-applet/device-allocator").path
    }
    ...
    static func resolveNode() -> String? {
        if let env = ProcessInfo.processInfo.environment["ARGENT_NODE"],
