...
public enum CoreAssets {
    ...
    private static func candidateDirs() -> [URL] {
        var dirs: [URL] = []
        if let env = ProcessInfo.processInfo.environment["CO_MAINTAINER_CORE"] {
        ...
        // Sources/CoMaintainerCore/CoreAssets.swift -> repo root is three levels up.
    ...
    /// The Co-Maintainer Mesh model (duty catalog, placement strategies, tier/token vocabulary)
