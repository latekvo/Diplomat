...
# `diplomat_app/__init__` installs this host, so this module is on the import path of
# every entry point either front-end has, including the stdlib-only node daemon — and a
# module that pulled the whole runtime in at import time would put all of it between the
# daemon's `python -m` and its first line of work.
