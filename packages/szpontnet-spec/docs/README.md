...
**Specification v0.9.0** (wire `v: 1`). This directory is the normative specification for
...
[`szpontnet-core/szpontnet/`](../../szpontnet-core/szpontnet) is its reference implementation; its
canonical constants live in [`netmodel.json`](../../szpontnet-core/szpontnet/netmodel.json). This spec is written so that a second,
...
| **v0.5.0** | [Tor transport](14-tor-transport.md) — WAN reachability with no public IP or domain, **on by default**: each node runs a permanent v3 onion service, advertises its `.onion` inside its signed advert, and redials known-but-unseen peers over Tor with exponential backoff (or a manual `--tor-connect` paste). Complementary to the LAN rather than an alternative to it — the LAN still carries every peer on this network, and this joins several networks into one mesh. A Tor link runs the identical handshake/trust as a LAN link. Additive: `v` stays `1`, and a node with it off (`SZPONTNET_TOR=0`, or no `tor` binary) is wire-unchanged. Also [claim re-assertion](12-work-claims.md#re-assertion) — a node re-announces its own active leases when a link binds and when an authoritative peer claims a key it owns, so two halves of a healed partition converge on one owner instead of both running the work. |
| **v0.6.0** | [Iroh transport](15-iroh-transport.md) — a **second** WAN transport beside [Tor](14-tor-transport.md), reaching the same peers with no public IP, no domain and no daemon: each node binds a permanent QUIC endpoint whose id is an Ed25519 public key, advertises that id inside its signed advert, and redials known-but-unseen peers by identity with exponential backoff (or a manual `--iroh-connect` paste). Complementary to the LAN rather than an alternative to it, and a link over it runs the identical handshake/trust as a LAN link. **Opt-in** (`SZPONTNET_IROH=1`), so Tor stays the default and a node may run either, both, or neither. One shared cache (`wan.json`) and reconnect policy now govern every WAN transport. Additive: `v` stays `1`, and a node with both off is wire-unchanged. |
| **v0.7.0** | [The preferred WAN transport](06-coordination.md#the-preferred-wan-transport) — which of [Tor](14-tor-transport.md) and [iroh](15-iroh-transport.md) an off-LAN edge settles on becomes a **mesh-wide** setting, gossiped in the placement-overrides record (new `wan` field, `set-wan` command) and last-writer-wins like a duty edit. It **orders, it never excludes**: every node still runs every transport it can, so the pick decides an edge only where both ends speak both, and a pair sharing exactly one still links over that one. The [snapshot](08-state.md#the-statejson-snapshot) reports each edge's agreed transport (`""` where the pair shares none) and this node's own address on each transport, so an operator can hand a peer an id and paste one back: [`connect`](04-messages.md#connect) dials a WAN id with the transport read off its shape. Additive: `v` stays `1`, and a record with no `wan` is byte-identical to a v0.6.0 one. |
| **v0.8.0** | [The iroh transport](15-iroh-transport.md) is **on by default**, the same as [Tor](14-tor-transport.md) — `SZPONTNET_IROH=0` turns it off, and the lenient off-list (`false`/`no`/`off`/empty) applies to both. A node that publishes an onion to peers everywhere gains nothing from making the cheaper, daemon-free address the one an operator has to go looking for, and the exposure is the same either way. Availability still gates it: an implementation whose iroh library is absent runs without the transport, which is what lets the default be on while the dependency stays optional. Nothing on the wire moves — `v` stays `1`, `endpoint` is still omitted by a node not running the transport, and a node with both transports off is byte-identical to a v0.7.0 one. |
| **v0.9.0** *(this revision)* | [Framing](03-transport.md#framing) - a line within the length limit that the receiver's own parser still refuses (nested past its stack, an integer literal past its digit limit) is dropped like any other malformed line, with the link kept; the conformance [V2 drop-set](10-conformance.md#v2---codec-round-trips) enumerates both. Nothing on the wire moves - `v` stays `1`. |
...
| 14 | [Tor transport](14-tor-transport.md) | WAN reachability, on by default: a permanent onion service per node, onion exchange in the signed advert, reconnect with exponential backoff, and manual `--tor-connect` |
| 15 | [Iroh transport](15-iroh-transport.md) | the same reachability, on by default and without a daemon: a permanent QUIC endpoint per node, endpoint exchange in the signed advert, and manual `--iroh-connect` |
...
- Wire protocol & node: [`szpontnet-core/szpontnet/`](../../szpontnet-core/szpontnet)
  (`protocol.py`, `node.py`, `assign.py`, `identity.py`, `statefile.py`, `ctl.py`).
- Canonical constants & duty catalog: [`netmodel.json`](../../szpontnet-core/szpontnet/netmodel.json)
  (a deployment may overlay its own - see [`host.py`](../../szpontnet-core/szpontnet/host.py)).
- Interop-relevant behavior is covered by
  [`test_mesh_logic.py`](../../diplomat-platform/linux/tests/test_mesh_logic.py) (the
  placement function) and
  [`test_mesh_node.py`](../../diplomat-platform/linux/tests/test_mesh_node.py) (real
