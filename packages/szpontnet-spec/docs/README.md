...
| **v0.6.0** *(this revision)* | [Iroh transport](15-iroh-transport.md) — WAN reachability with no public IP, no domain and no daemon, **on by default** and now the supported one: each node binds a permanent QUIC endpoint whose id is an Ed25519 public key, advertises that id inside its signed advert, and redials known-but-unseen peers by identity with exponential backoff (or a manual `--iroh-connect` paste). Complementary to the LAN rather than an alternative to it, and a link over it runs the identical handshake/trust as a LAN link. The [Tor transport](14-tor-transport.md) is **deprecated** and off by default; a node running both prefers iroh, and one shared cache (`wan.json`) and reconnect policy now govern every WAN transport. Additive: `v` stays `1`, and a node with both off is wire-unchanged. |
...
| 14 | [Tor transport](14-tor-transport.md) | **deprecated**, off by default: the onion-service WAN transport iroh replaces, kept while established Tor meshes migrate |
| 15 | [Iroh transport](15-iroh-transport.md) | WAN reachability, on by default: a permanent QUIC endpoint per node, endpoint exchange in the signed advert, reconnect with exponential backoff, and manual `--iroh-connect` |
