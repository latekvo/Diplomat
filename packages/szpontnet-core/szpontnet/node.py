...
from . import (
    assign, banned, config, crypto, env, identity, irohnet, peercache,
    protocol, spawnjob, statefile, stats, tor, trust, usage, wancache,
)
...
# bytes. The exact byte construction is normative (see szpontnet-spec/docs/11).
_AUTH_CONTEXT = b"szpontnet-auth-v1:"

# WAN reconnect backoff — per known peer we hold a WAN address for but can't
# currently see on the LAN. Start small, grow geometrically to a ceiling, and reset
# the moment a WAN link actually binds. The LAN↔WAN quality gap is small, so probing
# is deliberately unhurried and a peer already linked (over ANY transport) is never
# probed — no aggressive switching. Kept per PEER rather than per peer-and-transport
# because a tick dials each due peer exactly once, over its most preferred available
# transport. See the WAN transport section.
_WAN_BACKOFF_MIN_SECS = 10.0
_WAN_BACKOFF_MAX_SECS = 600.0
_WAN_BACKOFF_FACTOR = 2.0
# How often the WAN reconnect loop wakes to check which known peers are due.
_WAN_REDIAL_TICK_SECS = 5.0
# Upper bound on one WAN dial before it's abandoned (and the backoff grows). Sized
# for the slowest transport: an iroh dial resolves in well under a second once the
# endpoint is online, but a cold onion connect legitimately takes double-digit
# seconds.
_WAN_DIAL_TIMEOUT_SECS = 30.0
...
class _LinkRefused(ValueError):
    ...
    hostile message must never drop a healthy link (szpontnet-spec/docs/09). A fence
...
class _PendingResult:
    ...
    banned for work we actually did (szpontnet-spec/docs/13)."""
...
class _Awaiting:
    ...
    (szpontnet-spec/docs/13#accountability-deadline-reminder-ban): the completion
...
@dataclass
class _WanBackoff:
    """Per-peer WAN reconnect schedule: when the next probe is due (monotonic) and
    the current interval — doubled on each miss up to a ceiling, and dropped entirely
    the moment a WAN link to the peer actually BINDS (a valid signed hello), not on a
    bare transport-level answer — so a reachable peer reconnects promptly while an
    address that accepts a connection but never links stays throttled. See the WAN
    transport section in :class:`MeshNode`."""

    next_attempt: float = 0.0
    interval: float = _WAN_BACKOFF_MIN_SECS
...
class _Wan:
    ...
    The node holds these in preference order and never names a concrete transport
    outside this record: ``name`` doubles as the link tag a peer's ``transport``
    field reports, ``field`` is the :class:`wancache.WanEntry` attribute holding
    this transport's address for a peer, and ``impl`` is anything with the
    ``start``/``stop``/``address``/``dial`` seam (:class:`irohnet.IrohTransport`,
    :class:`tor.TorTransport`). Retiring a transport is deleting its entry."""
...
class MeshNode:
    def __init__(self) -> None:
        ...
        # Known peers' PERMANENT WAN addresses (wan.json), learned from their
        ...
        # repeat tick / paste never opens a second connection to the same peer.
        # Addresses are unambiguous across transports (64-hex id vs `.onion`), so one
        # set covers them all.
        ...
        # work-claims section below and szpontnet-spec/docs/12-work-claims.md.
        ...
        # (szpontnet-spec/docs/12). Presence is also the local idempotency guard: a
        ...
        # Foreign zero-trust request/response bookkeeping (szpontnet-spec/docs/13):
    ...
    def info(self) -> NodeInfo:
        info = NodeInfo(
            ...
            endpoint=self._wan_address("iroh"),
            onion=self._wan_address("tor"),
            stats=self.stats.advertise(real_frac=self._real_quota_frac(),
                                       pace=self._token_pace),
        )
        return self._sign_advert(info)

    def _wan_address(self, name: str) -> str:
        """This node's advertised permanent address on the named WAN transport, or
        '' when that transport is off or not yet reachable. Rides inside the signed
        advert (bound to the device key)."""
        for w in self._wan:
            if w.name == name:
                return w.impl.address() or ""
        return ""
    ...
    async def start(self) -> None:
        ...
        # WAN transports are ATOMIC add-ons running BESIDE the LAN rather than
        # instead of it: each brings up a permanent, NAT-independent address (in the
    ...
    async def stop(self) -> None:
        ...
        if self._udp_send:
            ...
            self._udp_send = None
        # Close the WAN transports last, so any in-flight link teardown above still
        # had a live endpoint / SOCKS to close against.
        for w in self._wan:
            await w.impl.stop()
        self._wan = []
        self.iroh = None
    ...
    async def _wan_serve(self, w: _Wan) -> None:
        ...
        take tens of seconds for Tor). Once it is live, gossip our new advert so
        currently-linked peers record where to reach us after we part ways on the
        LAN."""
        def inbound(reader, writer):
            return self._on_wan_inbound(w.name, reader, writer)

        if await w.impl.start(inbound):
            self._bump_and_gossip()  # our advert now carries this transport's address

    async def _on_wan_inbound(self, name: str, reader: asyncio.StreamReader,
                              writer: asyncio.StreamWriter) -> None:
        """A connection arriving over one of our WAN transports. Tag it with the
        transport before handing it to the normal accept path, so the link is known
        not to be a LAN link even though it may land on loopback."""
        self._link_transport[writer] = name
        try:
            ...
            # line, a secret mismatch, a ctl session) so a tagged writer never leaks
            # the map. pop is idempotent, so the double-pop is harmless.
            self._link_transport.pop(writer, None)

    def _remember_wan(self, peer_id: str, endpoint: str, onion: str,
                      fingerprint: str) -> None:
        """Persist a peer's permanent WAN addresses (from its SIGNED advert). Written
        on change only; the WAN sibling of :meth:`_remember_peer`, bounded the same
        way so a churn of ids can't grow wan.json without limit."""
        entry = wancache.WanEntry(
            endpoint=irohnet.normalize_endpoint(endpoint),
            onion=tor.normalize_onion(onion),
            fingerprint=fingerprint)
        if not entry.reachable():
            return
        prev = self._wan_cache.get(peer_id)
        if prev == entry:
            ...
            self._wan_cache[peer_id] = self._wan_cache.pop(peer_id)
            return
        self._wan_cache.pop(peer_id, None)
        self._wan_cache[peer_id] = entry
        while len(self._wan_cache) > _MAX_PEER_CACHE:
            # Evict a FOREIGN entry first (the oldest such), so a churn of foreign
            # addresses — a single linked foreign peer can advertise many distinct
            # signed adverts — can't push out the addresses of the PERSONAL peers we
            # actually redial (an isolation DoS on our WAN reconnect). Oldest overall
            # only when every entry is personal.
            evicted = next((pid for pid, e in self._wan_cache.items()
                            if not self._wan_is_personal(pid, e)), None)
            if evicted is None:
                evicted = next(iter(self._wan_cache))
            del self._wan_cache[evicted]
            # An evicted peer is no longer a WAN redial target, so drop its backoff
            # too — otherwise _wan_backoff accretes orphaned entries under id churn
            # (the unbounded-growth class the caches are already bounded against).
            self._wan_backoff.pop(evicted, None)
        wancache.save(self._wan_cache)

    def _wan_is_personal(self, peer_id: str, entry: wancache.WanEntry) -> bool:
        """Whether a WAN-cache entry belongs to a peer we trust as PERSONAL — the
        gate for two decisions a linked FOREIGN peer must not get leverage over:
        (1) ORIGINATING an auto-dial — else a foreign peer that advertised an
        arbitrary, attacker-chosen address turns our node into a dial reflector
        (and leaks our signed hello to a destination it picked); and (2) surviving
        cache eviction — else a churn of foreign addresses evicts those of the
        personal peers we do redial. Prefer the live peer's verified classification;
        fall back to the fingerprint the address was signed-paired with, judged
        against the ban list then the allowlist / default-trust (so full-altruism,
        default ``personal``, keeps redialing everyone — the operator's explicit
        choice)."""
        peer = self.peers.get(peer_id)
    ...
    def _wan_reset_backoff(self, peer_id: str) -> None:
        """A WAN link to this peer actually BOUND — clear the backoff so a reachable
        peer that flaps reconnects promptly instead of waiting out a grown interval.
        Fired on link establishment (see _learn_node), not on a bare connect."""
        self._wan_backoff.pop(peer_id, None)

    def _wan_grow_backoff(self, peer_id: str) -> None:
        """Schedule the peer's next WAN probe further out, doubling the interval up
        to the ceiling. Called before each dial attempt (a link binding then clears
        it), so an address that never links is probed geometrically less often."""
        b = self._wan_backoff.get(peer_id) or _WanBackoff()
    ...
    def _wan_redial_targets(self, now: float) -> list[tuple[str, _Wan, str]]:
        """Known peers to probe over the WAN right now: we hold an address for them
        on a ready transport, we trust them as PERSONAL (we never auto-dial a foreign
        address — see _wan_is_personal), our id sorts below theirs (the same
        smaller-id-dials rule as the LAN, so exactly one side dials), they are neither
        linked nor already being dialed, and their backoff is due. A peer linked over
        ANY transport is skipped — that is the whole of "no aggressive switching": a
        live link is never disturbed."""
        out: list[tuple[str, _Wan, str]] = []
        for peer_id, entry in self._wan_cache.items():
            if not self.local.id < peer_id:
                continue
            # Only ORIGINATE a dial to a peer we trust as PERSONAL. Auto-dialing a
            # foreign, attacker-advertised address would let a linked foreign peer aim
            # our node at arbitrary third-party destinations — a dial reflector + a
            # leak of our signed hello. Foreign peers reach us INBOUND; we don't chase
            # them over the WAN. A manual paste (peer_id=None in _wan_dial) still
            # bypasses this for a deliberate one-shot introduction.
            if not self._wan_is_personal(peer_id, entry):
                continue
            chosen = self._wan_transport_for(entry)
            if chosen is None:
                continue
            w, address = chosen
            peer = self.peers.get(peer_id)
            if (peer is not None and peer.linked) or address in self._wan_dialing:
                continue
            b = self._wan_backoff.get(peer_id)
            if b is not None and now < b.next_attempt:
                continue
            out.append((peer_id, w, address))
        return out

    async def _wan_redial_loop(self) -> None:
        """Probe known-but-unseen peers over the WAN with per-peer exponential
        backoff. A no-op until at least one transport is up (each is brought up by
        its own ``_wan_serve``), and it never touches a peer that already has a live
        link."""
        while True:
            await asyncio.sleep(_WAN_REDIAL_TICK_SECS)
            now = time.monotonic()
            for peer_id, w, address in self._wan_redial_targets(now):
                task = asyncio.get_running_loop().create_task(
                    self._wan_dial(w, address, peer_id=peer_id),
                    name=f"mesh-{w.name}-dial-{peer_id[:6]}")
                self._dial_tasks.add(task)
                task.add_done_callback(self._dial_tasks.discard)

    async def _wan_dial(self, w: _Wan, address: str,
                        peer_id: str | None = None) -> None:
        """Open a WAN link to ``address`` over ``w`` and run it EXACTLY like a
        LAN-dialed link (same hello/auth/trust handshake, same message pump).
        ``peer_id`` (when known) dedups against an existing link and drives the
        reachability backoff; a manual paste passes None and dials unconditionally —
        reaching a peer you may never have met on the LAN."""
        address = w.normalize(address)
        if not address:
            return
        peer = self.peers.get(peer_id) if peer_id else None
        if (peer is not None and peer.linked) or address in self._wan_dialing:
            return
        self._wan_dialing.add(address)
        # Pre-schedule the NEXT probe before dialing: assume this attempt is a miss and
        # let a genuine WAN link BINDING clear it (in _learn_node). Resetting the
        # backoff on the bare transport-level answer instead would let an address that
        # connects but never links — a rotated join secret, a squatted/reassigned
        # address, an answer-then-drop — defeat the backoff entirely and thrash a
        # fresh (expensive) connection every tick forever. A manual paste (no peer_id)
        # is unthrottled.
        if peer_id:
            self._wan_grow_backoff(peer_id)
        try:
            try:
                reader, writer = await asyncio.wait_for(
                    w.impl.dial(address), timeout=_WAN_DIAL_TIMEOUT_SECS)
            except Exception:  # noqa: BLE001
                # Peer unreachable (offline, address unpublished, transport busy) —
                # the next probe is already scheduled above, so just let the loop
                # retry. A manual paste (no peer_id) simply reports nothing; the
                # operator can re-issue it. Deliberately every Exception, not a
                # tuple: each transport raises its own unreachability type (SOCKS
                # gives OSError, iroh raises IrohError, which is not an OSError) and
                # to this loop they all mean the same "try again later".
                return
            self._link_transport[writer] = w.name  # this link runs over the WAN
            self._send_hello(writer)
            try:
                await writer.drain()
            except (ConnectionError, OSError):
                ...
                writer.close()
                return
            # A WAN-dialed link is talking to whoever answered the address, so — as on
            # a LAN dial — nothing is trusted until the first message is a valid
            # hello. ``host`` is the WAN address: it shows in the snapshot as the
            # peer's address but is kept OUT of the LAN redial cache (see _learn_node).
            await self._run_link(reader, writer, address, authenticated=False)
        finally:
            self._wan_dialing.discard(address)
    ...
    async def _on_tcp_connection(
        ...
        if first.get("t") == "ctl":
            ...
            # over ANY WAN transport: a WAN address is advertised to every mesh peer
            # (and can be pasted around), so serving ctl over it would expose the full
            # node-control surface to anyone holding that address — and in an OPEN
            # mesh (no join secret, the documented home-LAN default) with no
            # authentication at all. Peer LINKS (`hello`) legitimately arrive over the
            # WAN; control sessions do not. An inbound WAN connection is tagged with
            # its transport by _on_wan_inbound BEFORE this runs, so refuse ctl on
            # anything that is not a LAN link — a new transport is then refused by
            # default rather than by being remembered here. See
            # szpontnet-spec/docs/15#security-notes.
            if self._link_transport.get(writer, "lan") != "lan":
                writer.close()
                return
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # (conformance rule 10 / szpontnet-spec/docs/09). Handlers normalize
    ...
    def _learn_node(
        ...
        if link_writer is not None:
            ...
            if peer.transport != "lan":
                # A real WAN link BOUND (not merely a transport-level answer) is the
                # true "this address is usable" signal — clear the reconnect backoff
                # so a reachable peer that flaps reconnects promptly. An address that
                # answers but never gets here (secret rotated, address squatted) keeps
                # its pre-scheduled backoff and is throttled, exactly as intended.
                # See _wan_dial.
                self._wan_reset_backoff(info.id)
            # A hello on the peer's OWN link (a direct LAN link or a manual paste) is
            # the one authenticated source of a peer's permanent WAN addresses —
            # persist them so we can redial after this link drops. (Deliberately NOT
            # from third-party gossip: addresses are remembered only for peers we've
            # actually met, matching the LAN "first sight" model.)
            if info.endpoint or info.onion:
                self._remember_wan(info.id, info.endpoint, info.onion,
    ...
    def _store_claim(self, rec: protocol.ClaimRecord) -> bool:
        ...
        # (szpontnet-spec/docs/12#security-properties). So at the cap an AUTHORITATIVE incoming
    ...
    async def dispatch(self, duty_id: str, prompt: str,
        ...
        if config.server_mode():
            ...
            # Origination dedup (szpontnet-spec/docs/12): only the leaderless surplus-first
    ...
    async def _dispatch_to(self, node_id: str, duty_id: str, prompt: str,
        ...
        # entry just expires. (szpontnet-spec/docs/13)
    ...
    def _maybe_arm_deadline(self, job_id: str, node_id: str, direct: bool) -> None:
        ...
        (szpontnet-spec/docs/13#the-completion-deadline). Personal executors are devices
    ...
    def _admit(self, job: Job, trust_level: str) -> tuple[str, str]:
        ...
        - ``"confined"`` — the requester is **foreign** but we have a confinement
          ...
          ([_run_confined], szpontnet-spec/docs/13);
        ...
        if trust_level == "banned":
            ...
            # is a favor, and favors end here (szpontnet-spec/docs/13#the-ban).
    ...
    def _spawn_local(self, job: Job) -> tuple[str, str, bool]:
        ...
        if wk:
            ...
            # free it when the agent's completion sentinel appears (szpontnet-spec/docs/12).
    ...
    # security contract (szpontnet-spec/docs/11 + the full flow in szpontnet-spec/docs/13):
    ...
    def _run_confined(self, job: Job, requester_id: str) -> tuple[str, str, bool]:
        ...
        wk = job.work_key
        # Idempotency + the executor-claim (szpontnet-spec/docs/12), mirroring _spawn_local:
        ...
        # one job-timeout window can't grow it without limit (szpontnet-spec/docs/13 — "both
        ...
        # `job-progress` answer instead of silence (szpontnet-spec/docs/13).
        ...
        if wk:
            ...
            # (szpontnet-spec/docs/12#security-properties). We only compute the sandboxed result
    ...
    def _emit_result(self, job_id: str, to_node: str, result_payload: dict) -> None:
        ...
        acked or its deadline (szpontnet-spec/docs/13)."""
    ...
    def _on_job_result(self, msg: dict, writer: asyncio.StreamWriter) -> None:
        ...
        if entry.reminded_at is not None and not bool(result.get("ok", False)):
            ...
            # promise (szpontnet-spec/docs/13#resolution-fulfilled-extended-or-banned).
    ...
    # section makes it binding (szpontnet-spec/docs/13#accountability-deadline-
    ...
    async def _ctl_command(self, msg: dict) -> dict:
        ...
        if t == "claim":
            # The origination claim gate, stand-alone (szpontnet-spec/docs/12): a client
    ...
    def snapshot(self) -> dict:
        ...
        return {
            ...
            # Foreign zero-trust request/response in flight (szpontnet-spec/docs/13):
