...
| `telemetry.json` | the Telemetry screen's model: where the ledger and the transcript-scan cursor live, how often a quota sample is taken and how much history is kept, the lookback ranges (7/14/30/60, default 14), the histogram/series resolution, the confidence level — and the seven metrics with their per-platform icon + tint. The arithmetic over the ledger is `../Sources/DiplomatCore/Telemetry.swift`, mirrored by `diplomat_app/telemetry.py` and diffed field-for-field by `diplomat-platform/linux/tests/test_telemetry_parity.py` |
...
- **Prompt assembly is single-sourced in Swift.** `buildPrompt` in `DiplomatCore`
  ...
  (`diplomat-platform/linux/diplomat_app/promptcore.py` → `../Sources/DiplomatCoreCLI`), so the two
