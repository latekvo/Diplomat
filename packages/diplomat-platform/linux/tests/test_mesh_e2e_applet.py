...
def _mesh_env(tmp: Path) -> dict:
    return {
        ...
        # shipped default is zero-trust/foreign — see szpontnet-spec/docs/11).
...
def test_applet_meshes_and_dispatches(tmp_path, monkeypatch):
    ...
    try:
        ...
        pytest.skip("diplomat-core binary not built (packages/diplomat-platform/linux/install/build-core.sh)")
