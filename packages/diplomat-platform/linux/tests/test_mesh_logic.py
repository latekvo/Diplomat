...
Offline, no sockets, no Qt. Run with ``python -m pytest packages/diplomat-platform/linux/tests`` or
dependency-free via ``python packages/diplomat-platform/linux/tests/test_mesh_logic.py``.
"""
...
def test_own_link_hello_rekeys_past_a_forged_inflated_epoch(tmp_path, monkeypatch):
    ...
    possession still re-keys and verifies (szpontnet-spec/docs/11 — only the own link may
...
def test_confined_executor_dedups_locally_but_never_claims_the_work_key(tmp_path, monkeypatch):
    ...
    (szpontnet-spec/docs/12#security-properties). We only compute the sandboxed result and
...
def test_non_object_job_result_is_dropped_not_crashing(tmp_path, monkeypatch):
    ...
    tuple and tears the link (szpontnet-spec/docs/13: a malformed reply is simply dropped)."""
    if not crypto.AVAILABLE:
        return
    acted = []
    monkeypatch.setattr(spawnjob, "run_result_handler", lambda p: acted.append(p))
...
def test_foreign_flood_cannot_starve_a_personal_claim(tmp_path, monkeypatch):
    ...
    its lease — szpontnet-spec/docs/12 "a foreign or keyless node can never deny you work". At
