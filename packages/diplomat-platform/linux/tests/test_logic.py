...
Pure, offline — no gh, no display. Run with: ``python -m pytest packages/diplomat-platform/linux/tests``
(or ``python packages/diplomat-platform/linux/tests/test_logic.py`` for a dependency-free smoke run).
"""
...
def test_poll_dispatch_does_not_self_deadlock_on_the_overlap_lock():
    ...
    s._spawn_tracked = lambda prompt, url, number, source: True
