...
Exec points at this package's ``diplomat`` launcher), so "update" is three steps:
fast-forward the checkout to its upstream, re-run ``install/build-core.sh`` to
refresh the installed diplomat-core binary, and start the launcher again. Relaunching is all
...
def repo_root() -> Path:
    ...
    if env:
        return Path(env)
    # packages/diplomat-platform/linux/diplomat_app/selfupdate.py, so parents[4] = <repo>.
    return Path(__file__).resolve().parents[4]


def linux_package(root: Path) -> Path:
    """This applet's package inside a checkout — the one place the layout is spelled,
    so the build script and the launcher below can only be looked for together."""
    return root / "packages" / "diplomat-platform" / "linux"
...
def build_core() -> None:
    ...
    """
    root = repo_root()
    script = linux_package(root) / "install" / "build-core.sh"
