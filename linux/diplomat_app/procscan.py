"""Process identification and newest-wins reaping, shared by the two singletons.

The tray (:mod:`diplomat_app.singleton`) and the mesh node
(:mod:`diplomat_app.mesh.singleton`) both enforce "newest wins" over a class of
long-lived process: on startup, find every *other* live instance of my own kind
and terminate it. Only the *identity test* differs between them (a tray is
``python -m diplomat_app``; a node is ``python -m diplomat_app.mesh`` with no
flags). Everything around that test — reading a pid's argv, walking ``/proc``
restricted to this uid, and the SIGTERM/grace/SIGKILL escalation — is the same
routine, and it is the half that decides which process receives a signal.

Keeping one copy is a safety property, not tidiness: both callers reap by
sending SIGKILL to pids they inferred. A guard added to one copy and missed in
the other (the recycled-pid check is exactly such a guard) means the copy that
missed it force-kills an unrelated process of the same user.
...
def module_arg(tokens: list[str]) -> str | None:
    ...
    look-alike top-level (``diplomat_app.meshery``) nor a deeper submodule
    (``diplomat_app.mesh.ctl``) can pass as the module it resembles.
    """
    try:
        i = tokens.index("-m")
    except ValueError:
