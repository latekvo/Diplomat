...
Mirrors ``Sources/CoMaintainer/DeviceAllocator.swift``: the applet is a *viewer*
...
def package_dir() -> str:
    ...
    env = os.environ.get("CO_MAINTAINER_DEVICE_ALLOCATOR_DIR")
    if env:
        return env
    ...
    # file is <repo>/linux/co_maintainer/deviceallocator.py, so parents[2] = <repo>.
    sibling = Path(__file__).resolve().parents[2] / "device-allocator"
    if sibling.exists():
        return str(sibling)
    return str(_home() / "dev" / "co-maintainer-applet" / "device-allocator")
...
def resolve_node() -> str | None:
    ...
    env = os.environ.get("CO_MAINTAINER_NODE")
...
def resolve_npm() -> str | None:
    ...
    env = os.environ.get("CO_MAINTAINER_NPM")
    if env and os.path.exists(env):
        return env
    node = resolve_node()
    if node:
