...
def _candidate_dirs() -> list[Path]:
    cands: list[Path] = []
    env = os.environ.get("CO_MAINTAINER_CORE")
    if env:
        cands.append(Path(env))
    # Repo layout: <repo>/linux/co_maintainer/core.py -> <repo>/core
...
def mesh() -> dict:
    ...
    (``co_maintainer.mesh``) and the topology panel.
    """
...
def audit_categories() -> dict:
    ...
    Mirrors Sources/CoMaintainerCore/AuditCategory.swift; see
