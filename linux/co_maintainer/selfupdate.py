"""Self-update: fast-forward the checkout, rebuild co-maintainer-core, relaunch.

The Linux applet runs straight out of its git checkout (the autostart .desktop
Exec points at ``linux/co-maintainer``), so "update" is three steps: fast-forward
the checkout to its upstream, re-run ``scripts/build-core.sh`` to refresh the
installed co-maintainer-core binary, and start the launcher again. Relaunching is all
...
def repo_root() -> Path:
    ...
    env = os.environ.get("CO_MAINTAINER_SELF_REPO")
    if env:
        return Path(env)
    # This file is <repo>/linux/co_maintainer/selfupdate.py, so parents[2] = <repo>.
...
def pull() -> str:
    ...
    if not _has_git_identity(root):
        ident = ["-c", "user.name=Co-Maintainer updater",
                 "-c", "user.email=co-maintainer@localhost"]
...
def build_core() -> None:
    ...
    Rebuilds the static co-maintainer-core CLI and installs it to
    ``~/.local/share/co-maintainer/co-maintainer-core`` — the actual "reinstall" step;
...
def relaunch(extra_env: dict[str, str] | None = None) -> None:
    ...
    """
    root = repo_root()
    launcher = root / "linux" / "co-maintainer"
    log_dir = _state_dir()
    env = dict(os.environ)
    if extra_env:
    ...
    try:
        ...
        with (log_dir / "co-maintainer.log").open("ab") as log:
...
def run_scheduled() -> int:
    ...
    co-maintainer-core — then relaunches the tray only if one is actually running (so it
    ...
    _sched_log(f"merged to {commit} — rebuilding co-maintainer-core")
    try:
        build_core()
    except UpdateError as exc:
