...
Mirrors the macOS ``Dump`` enum so the two front-ends can be cross-checked:
    ...
    CO_MAINTAINER_DUMP=1           full fetch+filter pipeline, prints all 6 tools
    CO_MAINTAINER_LOOKUP=337       reverse-lookup one number through the real Store
    CO_MAINTAINER_PRINT_PROMPT=... assemble + print a Review-PRs prompt (mine|user|single)
