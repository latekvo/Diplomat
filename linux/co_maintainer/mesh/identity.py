...
def mesh_dir() -> Path:
    """State directory — override with CO_MAINTAINER_MESH_DIR (tests give every
    fake node its own)."""
    env = os.environ.get("CO_MAINTAINER_MESH_DIR")
...
def detect_platform() -> str:
    env = os.environ.get("CO_MAINTAINER_MESH_PLATFORM")  # tests fake mixed-OS meshes
    if env:
        return env
