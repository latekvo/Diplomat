...
tier (:func:`co_maintainer.mesh.identity.load`). A manual edit in the panel pins
...
def detect_tier() -> int:
    ...
    1 = strongest). ``CO_MAINTAINER_MESH_TIER`` forces a value (tests / manual pinning at
    the process level); an undetectable box falls back to ``tiers.default``."""
    lo, hi, default = config.tier_bounds()
    forced = os.environ.get("CO_MAINTAINER_MESH_TIER")
