...
1. ``CO_MAINTAINER_MESH_SPAWN`` — a command template (``{prompt_file}`` substituted,
...
2. macOS — ``osascript`` opens Terminal.app on the same shell command the
   Linux spawner uses.
3. Linux — the applet's own terminal spawner (``co_maintainer.review.spawn``),
...
``CO_MAINTAINER_MESH_FOREIGN_SPAWN``), the result is written to a file the node returns
...
_CONFINED_PREAMBLE = (
    ...
    "and write it to the file named by $CO_MAINTAINER_MESH_RESULT_FILE (write it in one shot "
...
def _fill(template: str, **subs: str) -> str:
    ...
    the personal ``CO_MAINTAINER_MESH_SPAWN`` shape)."""
    cmd = template
...
def _spawn_override(prompt_file: str, template: str) -> None:
    _detached(_fill(template, prompt_file=prompt_file), "CO_MAINTAINER_MESH_SPAWN")
...
def spawn_job(prompt: str) -> str:
    ...
    template = os.environ.get("CO_MAINTAINER_MESH_SPAWN")
    if template:
...
def spawn_confined(prompt: str, result_file: str) -> str:
    ...
    inside the operator's sandbox — ``CO_MAINTAINER_MESH_FOREIGN_SPAWN``, with
    ``{prompt_file}``/``{result_file}`` substituted and also exported as
    ``CO_MAINTAINER_MESH_PROMPT_FILE``/``CO_MAINTAINER_MESH_RESULT_FILE`` — under a credential-
    ...
    template = os.environ.get("CO_MAINTAINER_MESH_FOREIGN_SPAWN", "")
    if not template:
        # Belt and braces: the caller only reaches here when a runner is configured.
        raise JobSpawnError("no confinement runner (CO_MAINTAINER_MESH_FOREIGN_SPAWN unset)")
    prompt_file = review.write_prompt(_CONFINED_PREAMBLE + prompt)
    env = _scrubbed_env(
        CO_MAINTAINER_MESH_CONFINED="1",
        CO_MAINTAINER_MESH_PROMPT_FILE=prompt_file,
        CO_MAINTAINER_MESH_RESULT_FILE=result_file,
    )
    _detached(_fill(template, prompt_file=prompt_file, result_file=result_file),
              "CO_MAINTAINER_MESH_FOREIGN_SPAWN", env=env)
    return prompt_file


def run_result_handler(result_file: str) -> None:
    """Hand a returned ``job-result`` to the originator's own result handler —
    ``CO_MAINTAINER_MESH_ON_RESULT`` with ``{result_file}`` substituted (and exported as
    ``CO_MAINTAINER_MESH_RESULT_FILE``). This is where the **social action runs under the
    ...
    if not template:
        return
    _detached(_fill(template, result_file=result_file), "CO_MAINTAINER_MESH_ON_RESULT",
              env={**os.environ, "CO_MAINTAINER_MESH_RESULT_FILE": result_file})
