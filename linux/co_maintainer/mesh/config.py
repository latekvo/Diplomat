...
2. ``CO_MAINTAINER_MESH_*`` environment overrides for the protocol knobs — how the
...
_ENV_KEYS = {
    "CO_MAINTAINER_MESH_MCAST_GROUP": "multicastGroup",
    "CO_MAINTAINER_MESH_MCAST_PORT": "multicastPort",
    "CO_MAINTAINER_MESH_TCP_BASE": "tcpPortBase",
    "CO_MAINTAINER_MESH_TCP_SPAN": "tcpPortSpan",
    "CO_MAINTAINER_MESH_BEACON_SECS": "beaconIntervalSecs",
    "CO_MAINTAINER_MESH_REDIAL_SECS": "redialIntervalSecs",
    "CO_MAINTAINER_MESH_HEARTBEAT_SECS": "heartbeatIntervalSecs",
    "CO_MAINTAINER_MESH_STALE_SECS": "peerStaleSecs",
    "CO_MAINTAINER_MESH_TIMEOUT_SECS": "peerTimeoutSecs",
    "CO_MAINTAINER_MESH_ACK_SECS": "dispatchAckTimeoutSecs",
    "CO_MAINTAINER_MESH_STATE_SECS": "stateWriteIntervalSecs",
    "CO_MAINTAINER_MESH_RESULT_RETRY_SECS": "foreignResultRetryIntervalSecs",
    "CO_MAINTAINER_MESH_RESULT_MAX_SECS": "foreignResultMaxSecs",
    "CO_MAINTAINER_MESH_FOREIGN_TIMEOUT_SECS": "foreignJobTimeoutSecs",
}


def protocol() -> dict:
    """The protocol constants with any CO_MAINTAINER_MESH_* env overrides applied."""
...
def loopback_only() -> bool:
    """CO_MAINTAINER_MESH_LOOPBACK=1 keeps every socket on 127.0.0.1 — used by the
    integration tests (and demos) to run a whole mesh on one machine without
    touching the real LAN."""
    return os.environ.get("CO_MAINTAINER_MESH_LOOPBACK") == "1"


def secret() -> str:
    """Optional pre-shared join token (CO_MAINTAINER_MESH_SECRET, same value on every
    ...
    """
    return os.environ.get("CO_MAINTAINER_MESH_SECRET", "")


def server_mode() -> bool:
    """CO_MAINTAINER_MESH_SERVER=1 makes this node a dedicated **server**: it accepts and
    ...
    return os.environ.get("CO_MAINTAINER_MESH_SERVER") == "1"


def api_key() -> str:
    """Optional per-node API key (CO_MAINTAINER_MESH_API_KEY). When set, this node
    ...
    return os.environ.get("CO_MAINTAINER_MESH_API_KEY", "")


def foreign_spawn() -> str:
    """Optional **confinement runner** (CO_MAINTAINER_MESH_FOREIGN_SPAWN) — the command
    ...
    return os.environ.get("CO_MAINTAINER_MESH_FOREIGN_SPAWN", "")


def on_result() -> str:
    """Optional **result handler** (CO_MAINTAINER_MESH_ON_RESULT) — the command template an
    ...
    return os.environ.get("CO_MAINTAINER_MESH_ON_RESULT", "")
