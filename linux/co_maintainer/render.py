...
a real display by grabbing the widget's own pixels:
    ...
    CO_MAINTAINER_RENDER=panel CO_MAINTAINER_RENDER_OUT=/tmp/p.png \
        QT_QPA_PLATFORM=offscreen python -m co_maintainer

what ∈ {panel, lookup, wizard, conflicts, settings, devices, mesh}. With
CO_MAINTAINER_RENDER_LIVE=1 it fetches real data first; otherwise it uses a small
synthetic fixture.
"""
...
def run(what: str, out: str) -> int:
    ...
    store = Store()
    if os.environ.get("CO_MAINTAINER_RENDER_LIVE") == "1":
        store.refresh()
    else:
        _fixture(store)
