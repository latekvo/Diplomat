...
(CoMaintainerCore/Review.swift) and are reached by shelling out to the
``co-maintainer-core`` CLI, so the two front-ends can't drift. ``ReviewConfig`` mirrors
...
class SpecificAuthor(Enum):
    ...
    ``SpecificAuthor`` enum in CoMaintainerCore/Review.swift). Selects the prompt
...
class ReviewConfig:
    ...
    def build_prompt(self) -> str:
        # Single-sourced in Swift (CoMaintainerCore) — assembled by the co-maintainer-core
...
def repo_path() -> str:
    """The local checkout the agent works in (override with CO_MAINTAINER_REPO)."""
    return os.environ.get("CO_MAINTAINER_REPO") or os.path.expanduser("~/dev/argent")
...
def write_prompt(prompt: str) -> str:
    ...
    try:
        fd, path = tempfile.mkstemp(prefix="co-maintainer-review-", suffix=".txt")
...
def user_shell() -> str:
    ...
    CO_MAINTAINER_SHELL; falls back to $SHELL, then bash."""
    return os.environ.get("CO_MAINTAINER_SHELL") or os.environ.get("SHELL") or "/bin/bash"
