...
class Store(QObject):
    ...
    def mesh_enabled(self) -> bool:
        ...
        Co-Maintainer never opens a UDP/TCP node on the network unasked; the app
    ...
    def _spawn_tracked(self, prompt: str, url: str, number: int) -> bool:
        ...
        fd, done_path = tempfile.mkstemp(prefix="co-maintainer-autofix-done-", suffix=".txt")
        os.close(fd)
        try:
    ...
    def ensure_allocator_installed_async(self) -> None:
        """One-time automatic install of the device-allocator MCP when Co-Maintainer
    ...
    def update_applet_async(self) -> None:
        """Pull the checkout, rebuild co-maintainer-core, relaunch the applet.
        ...
        def work() -> None:
            ...
            try:
                ...
                step(f"building co-maintainer-core at {commit}…")
        ...
        def work() -> None:
            ...
            try:
                subprocess.Popen(  # noqa: S603 — relaunch ourselves as a node
                    [sys.executable, "-m", "co_maintainer.mesh", "--daemon"],
                    cwd=linux_dir,
