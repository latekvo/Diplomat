...
def test_a_namespace_directory_does_not_count_as_the_library(monkeypatch, tmp_path):
    """Run from the repo root, the *project* directory ``szpontnet/`` — which holds
    ``docs/`` and ``conformance/`` beside the package — is itself importable as a
    namespace package. It resolves, has no ``__init__`` and exports nothing, so
    treating "a spec exists" as "the library is here" reports it present and then
    dies on the first real import with "unknown location"."""
    (tmp_path / "szpontnet").mkdir()  # a project dir, with no package inside it
